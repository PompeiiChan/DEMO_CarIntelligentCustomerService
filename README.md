...
用户输入不会直接丢给大模型生成答案，而是先经过一层客服路由 Pipeline。系统会先做确定性安全拦截，再用分类 Prompt 识别业务路由、一级意图、置信度、澄清需求和情绪，最后根据结果选择“澄清反问、RAG 回答、转人工”中的一种路径。
...
flowchart TD
  ...
  B --> C{"确定性门禁"}
  C -->|"明确要求人工<br/>转人工 / 找真人"| H1["直接转人工<br/>reason=用户要求"]
  C -->|"高风险售后<br/>抛锚 / 无法启动 / 充不了电 / 事故 / 救援"| H2["直接转人工<br/>reason=高风险售后<br/>priority=high"]
  C -->|"普通咨询"| D["分类 Prompt<br/>只返回 JSON"]
